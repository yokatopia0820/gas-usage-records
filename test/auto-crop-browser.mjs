...
const appUrl = process.env.APP_URL || 'https://yokatopia0820.github.io/gas-usage-records/?v=manual-v3';
...
test('upload opens the persistent LCD range selector before local OCR', async () => {
  ...
  assert.equal(state.crop, true, JSON.stringify(state));
  assert.equal(state.button, true, JSON.stringify(state));
  assert.equal(state.overlay, false);
  assert.equal(state.preview, false, JSON.stringify(state));
  assert.equal(state.weight, '', JSON.stringify(state));
});
